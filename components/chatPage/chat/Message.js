...
import { Side, MessageType } from '../../../constants.js';
...
export const GetMessage = ({ userStore, message }) => {
    if (message.type !== MessageType.PlainText) throw new Error(`ContentType ${message.type} not implemented`);
    const user = userStore.getUserById(message.userId);
    const senderImage = `<div><img class="avatar small" src="${user.avatarUri ?? 'public/default-avatar.jpg'}" alt="avatar1 profile picture"></div>`;
    const senderName = `<div class="from-display-name">${user.username ?? '<i>anonymous</i>'}</div>`;
    
    const node = htmlToElement(
        `<div class="line message ${message.side}">
            <div class="with-user-info ${message.side}">
                ${message.side === Side.RECEIVED? senderImage: ''}
                <div class="bubble" style="position: relative;">
                    ${message.side === Side.RECEIVED? senderName: ''}
                    <div class="content support-emojis">${message.content}</div>
                    <div class="meta">
                        <div class="time">${timestampToTime(message.timestamp)}</div>
                        ${message.side === Side.SENT ? StatusMark({ status: 'received' }) : ''}
                    </div>
                </div>
            <div>
        </div>`
    ...
    if (message.side === Side.RECEIVED) {
        const avatar = node.querySelector('.avatar');
        const username = node.querySelector('.from-display-name');
        userStore.subscribeOnUserUpdated(message.userId, ( { detail }) => {
