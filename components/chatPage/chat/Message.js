...
import { Side, MessageContentType } from '../../../constants.js';
...
export const GetMessage = ({ userStore, userId, content, timestamp, status, side, contentType }) => {
    if (contentType !== MessageContentType.PlainText) throw new Error(`ContentType ${contentType} not implemented`);
    const user = userStore.getUserById(userId);
    const senderImage = `<div><img class="avatar small" src="${user.avatarUri ?? 'public/avatars/default-avatar.jpg'}" alt="avatar1 profile picture"></div>`;
    const senderName = `<div class="from-display-name">${user.displayName ?? '<i>anonymous</i>'}</div>`;
    
    const node = htmlToElement(
        `<div class="line message ${side}">
            <div class="with-user-info ${side}">
                ${side === Side.RECEIVED? senderImage: ''}
                <div class="bubble" style="position: relative;">
                    ${side === Side.RECEIVED? senderName: ''}
                    <div class="content support-emojis">${content}</div>
                    <div class="meta">
    ...
    if (side === Side.RECEIVED) {
        ...
        const displayName = node.querySelector('.from-display-name');
        userStore.subscribeOnUserUpdated(userId, ( { detail }) => {
