...
import { Side, ChatType, MessageContentType } from '../../../enums.js';
...
export const GetMessage = ({ userStore, userId, content, timestamp, status, side, contentType, chatType }) => {
    const isChatGroup = chatType === ChatType.Group;
    if (contentType !== MessageContentType.PlainText) throw new Error(`ContentType ${contentType} not implemented`);
    const user = userStore.getUserById(userId);
    const senderImage = `<div><img class="avatar small" src="${user.avatarUri}" alt="avatar1 profile picture"></div>`;
    const senderName = `<div class="from-display-name">${user.displayName}</div>`;
    ...
    const node = htmlToElement(
        `<div class="line message ${side}">
            <div class="with-user-info ${side}">
                ${side === Side.RECEIVED && isChatGroup? senderImage: ''}
                <div class="bubble" style="position: relative;">
                    ${side === Side.RECEIVED && isChatGroup? senderName: ''}
